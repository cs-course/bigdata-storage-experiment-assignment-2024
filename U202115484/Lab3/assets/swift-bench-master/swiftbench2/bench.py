...
sys.path.append('swift-bench-master')
from swiftbench2.utils import config_true_value, using_http_proxy, \
    get_size_bytes
...
class Bench(object):
    ...
    def _log_status(self, title):
        ...
        self.logger.info('total is %(total)f',{'total':total})
        self.logger.info(
            '%(complete)s %(title)s [%(fail)s failures], %(rate)f/s',
    ...
    def run(self):
        ...
        self.logger.info("concurrency"+str(self.concurrency))
        ...
        pool.waitall() 
        wholetime=time.time()-self.beginbeat
        self._log_status(self.msg + ' **FINAL**')
        with open('data/'+self.msg+'_'+str(self.object_size)+'_'+str(self.concurrency)+'_'+str(self.total)+'.csv','w') as file:
            file.write("ID,start,end,latency\n")
            self.result.sort(key= lambda x:x[0])
            for res in self.result:
                file.write(str(res[0])+','+str(res[1]*1000)+','+str(res[2]*1000)+','+str(res[3]*1000)+'\n')
            self.logger.info("time is "+str(wholetime))
            file.write(str(self.object_size*self.total/wholetime))
    ...
    def _run(self, thread):
        ...
        with self.connection() as conn:
            try:
                ...
                self.failures += 1
        aftertime=time.time()
        # self.logger.info("%(thread)d %(time)f",{'thread':thread,"time":aftertime-beforetime})
        self.result.append((thread,aftertime,beforetime,aftertime-beforetime))
        self.complete += 1
