...
use rusoto_s3::{
    ...
    ListObjectsV2Error, ListObjectsV2Output, ListObjectsV2Request, PutObjectRequest, S3Client, S3,
};
use tokio::{
    fs::{remove_file, File},
...
pub async fn delete_bucket_all(cli: Arc<Client>, bucket: String) {
    let resp = cli.list_bucket_all(bucket.clone()).await;
        ...
        Ok(res) => {
            if let Some(objects) = res.contents {
                for object in objects {
                    if let Err(_) = cli
                        .delete_object(bucket.clone(), object.key.unwrap().as_str())
                        .await
                    {
                        panic!("delete object error");
                    }
                }
            }
        }
        Err(e) => {
            panic!("list bucket all err: {}", e);
        }
    }
}
...
impl Client {
    ...
    pub async fn list_bucket_all(
        ...
        self.cli.list_objects_v2(list_objects_request).await
    }
