...
impl S3Client {
    ...
    pub async fn create_bucket(&self, bucket: &str) -> Result<(), Box<dyn Error>> {
        let resp = self.s3_client.create_bucket().bucket(bucket).send().await?;
    ...
    #[allow(dead_code)]
    pub async fn list_objects(&self, bucket: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        let resp = match self.s3_client.list_objects_v2().bucket(bucket).send().await {
            Ok(resp) => resp,
            Err(e) => {
