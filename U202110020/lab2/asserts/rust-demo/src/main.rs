...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.cmd {
        ...
        Commands::GetObject { bucket, object, destination } => {
            let byte_count = s3_client.get_object(bucket.as_str(), object.as_str(), &destination).await?;
            println!("downloaded {} bytes", byte_count);
            println!("get object {} success", destination.display());
        },
        Commands::PutObject { bucket, object, source } => {
            s3_client.put_object(bucket.as_str(), object.as_str(), &source).await?;
            println!("put object {} success", source.display());
        },
        Commands::DeleteObject { bucket, object } => {
            s3_client.delete_object(bucket.as_str(), object.as_str()).await?;
            println!("delete object {} success", object);
        },
        Commands::CopyObject { source_bucket, source_object, destination_bucket, destination_object } => {
            s3_client.copy_object(source_bucket.as_str(), source_object.as_str(), destination_bucket.as_str(), destination_object.as_str()).await?;
            println!("copy object {} to {} success", source_object, destination_object);
        }
    }
...
mod tests {
    ...
    async fn test_list_objects() -> Result<(), Box<dyn Error>>{
        let s3_client = client::S3Client::new().await;
        s3_client.list_objects("test-1").await?;
        Ok(())
    }
    ...
    async fn test_get_object() -> Result<(), Box<dyn Error>>{
        let s3_client = client::S3Client::new().await;
        s3_client.get_object("test-1", "test.txt", &PathBuf::from("./files/test.txt")).await?;
        Ok(())
    }
    ...
    async fn test_put_object() -> Result<(), Box<dyn Error>>{
        let s3_client = client::S3Client::new().await;
        s3_client.put_object("test-1", "test.txt", &PathBuf::from("./files/1.txt")).await?;
        Ok(())
    }
    ...
    async fn test_copy_object() -> Result<(), Box<dyn Error>>{
        let s3_client = client::S3Client::new().await;
        s3_client.copy_object("test-1", "test.txt", "test-2", "test.txt").await?;
        Ok(())
    }
}