  ...
  public:
    InternalCuckooMap(bool useMmap, uint64_t size,
        ...
        : _randState(0x2636283625154737ULL),
          ...
          _valueOffset(sizeof(Key)), _useMmap(useMmap), _nrUsed(0) {
        ...
        if (_useMmap) {
            ...
            _base = reinterpret_cast<char *>(
                ...
                ~((uintptr_t)0x3fu)); // to actually implement the 64-byte
                                      // alignment, shift base pointer within
                                      // allocated space to 64-byte boundary
        }
        try {
    ...
    int insert(Key &k, Value *v, Key **kPtr, Value **vPtr) {
        ...
        //
        // If kPtr and vPtr and non-null pointers, and the return is
        // non-negative, the position of k and v in the table are written to
        // *kPtr and *vPtr.
        //
        ...
        // mispredicted branch in the first loop:
        uint64_t hash2 = _hasher2(k);
        uint64_t pos2 = hashToPos(hash2);
    ...
    bool expungeRandom(Key &k, Value *v) {
        ...
        // returned
        // and the table is unchanged, in this case k and *v are
        // also unchanged. Otherwise, true is returned and k and *v are
        // overwritten with the values of the expunged pair and 1 is returned.
